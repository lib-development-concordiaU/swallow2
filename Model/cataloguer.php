...
class Cataloguer extends db{
	...
	public function create($in_name, $in_lastname, $in_email, $in_pwd,$in_role = 0){
		$sql = "SELECT * FROM cataloguer WHERE email='".$in_email."'";
		$result = $this->conn->query($sql);
		if ($result->num_rows == 0) {
			$sql = "INSERT INTO cataloguer (name,lastname,email,pwd,role) VALUES ('". $in_name ."','". $in_lastname ."','". $in_email . "','" . $in_pwd . "',".$in_role.")";
	...
	public function save(){
		...
		$sql = "UPDATE cataloguer SET name='".trim($this->name)."', lastname='".trim($this->lastname)."',email='".trim($this->email)."', pwd = '".trim($this->pwd)."' , role = ".$this->role." WHERE id=$this->id";
